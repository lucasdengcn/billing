...
/**
 * Pricing strategy for usage-based products and subscriptions.
 */
public class UsageBasedPricingStrategy implements PricingStrategy {
    ...
    public BigDecimal calculateProductPrice(Product product) {
        ...
        // Ensure this strategy is used for usage-based products
        if (product.getPriceType() != PriceType.USAGE_BASED && product.getPriceType() != null) {
            throw new IllegalArgumentException("UsageBasedPricingStrategy should only be used for USAGE_BASED products");
        }
        
        BigDecimal basePrice = product.getBasePrice();
        BigDecimal discountRate = product.getDiscountRate() != null ? product.getDiscountRate() : DEFAULT_DISCOUNT_RATE;
        
        // For usage-based products, the price might include a base fee plus usage charges
    ...
    public BigDecimal calculateSubscriptionPrice(Subscription subscription) {
        ...
        // For usage-based subscriptions, the price calculation might depend on usage metrics
        // For now, we'll use the base fee with discount, but in practice this could involve usage calculations
